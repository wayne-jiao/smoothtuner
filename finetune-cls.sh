...
LOCAL_RANK=1 && echo LOCAL_RANK: ${LOCAL_RANK}
NCCL_DEBUG=INFO
...
python -m torch.distributed.launch --nproc_per_node ${PER_NODE_GPU} --node_rank=${RANK} --nnodes=${NODES} --master_addr=${MASTER_HOST} --master_port=${MASTER_PORT} ../run_finetune_cls.py  \
  ...
  --output_dir ${mnt_dir}/save/saved_models_cls_shuai_scr \
  --train_filename ${mnt_dir}/Diff_CPP \
  --dev_filename ${mnt_dir}/Diff_CPP/cls-valid.jsonl \
